...
- **Hebrew/English toggle under xcb needs `setxkbmap` (pkg `x11-xkb-utils`, v1.5.12) AND a *post-focus* re-apply (v1.5.13).** As an X11 (xcb) client Konsole does layout switching via `setxkbmap`, not WSLg — so the installer must install `x11-xkb-utils` (it's the new hard dependency; a missing tool must LOG, not silently no-op). But applying the layout *before* the window has focus gets dropped on cold start → "won't switch on the first launch." Re-apply in the background a few seconds after the window is raised/focused (same slow-first-paint timing as the window), stopping once the RTL group is loaded so you don't reset the active group mid-use. Alt+Shift also collides with Windows' own language hotkey → keep it configurable (`KIVUN_KBD_TOGGLE` in config.txt; `grp:caps_toggle` etc.).
