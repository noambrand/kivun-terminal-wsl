...
- **Keyboard: plain WSLg switches Hebrew/English; FORCING xcb breaks it (the big lesson, v1.5.14).** On plain WSLg (Wayland) the user's normal Alt+Shift switches layouts — verified live on a real machine (window visible AND switching both work). v1.5.9 force-switched Konsole to XWayland (`QT_QPA_PLATFORM=xcb`) so xdotool/wmctrl could manage the window, but that turned Konsole into an X11 client whose keyboard is driven by `setxkbmap`, which collides with Windows' own Alt+Shift and **broke switching (the v1.5.9–v1.5.13 regression).** So **plain Wayland is the DEFAULT again; xcb is opt-in (`KIVUN_FORCE_XCB=on`)** only for the rare machine whose Wayland window opens minimized. Process lesson the user drove home: **a big display-backend change like Wayland↔xcb needs a clear question to the user first — don't quietly force it and misguide them.** (If xcb IS opted into, that path still needs `x11-xkb-utils`/`setxkbmap` + a post-focus re-apply gated to xcb, and `KIVUN_KBD_TOGGLE` only applies there.)
