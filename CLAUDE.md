...
- **Keyboard: XWayland (xcb) + `setxkbmap` is REQUIRED; plain Wayland CANNOT switch (verified TWICE from a clean state, v1.5.15).** Under WSLg, plain Wayland/Weston does NOT do live layout switching — the window shows but Alt+Shift does nothing. Switching works ONLY when Konsole is an X11 client (`QT_QPA_PLATFORM=xcb`) and the launcher runs `setxkbmap us,<rtl> -option grp:alt_shift_toggle` (needs `x11-xkb-utils`, applied early + a post-focus re-apply). Forcing xcb ALSO fixes the "window only in the taskbar" bug, so the SAME default fixes both. The last v1.4 "just worked" because older Ubuntu ran Konsole on X11 by default; newer Qt6 defaults to Wayland — THAT is the regression. So **xcb is the DEFAULT** (`KIVUN_FORCE_XCB=auto`); `off` = plain Wayland (window may hide, no switching). `<rtl>` comes from the configured language (hebrew→il, arabic→ara, …; `kivun-launch.sh` case), so it is NOT Hebrew-only.
- **Painful meta-lesson that cost v1.5.14: NEVER trust a keyboard test on a machine with leftover state.** `KIVUN_TABBED=on` makes a new launch TAB INTO an existing (old-backend) window, and a prior `setxkbmap` keymap lingers on `:0` — so a contaminated test shows "Alt+Shift works" on Wayland when a CLEAN test shows it fails. v1.5.14 flipped the default to Wayland on exactly that false positive and shipped broken; v1.5.15 restored xcb after a CLEAN test. Always verify keyboard/display changes only after **`wsl --shutdown` AND closing every Konsole window**. And a big display-backend flip (Wayland↔xcb) is a real architectural decision — put it to the user as a clear question, don't quietly force it.
