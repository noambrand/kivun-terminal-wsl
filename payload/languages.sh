...
kivun_lang_prompt() {
    ...
    case "$1" in
        hebrew)                  echo "Always respond in Hebrew. Optimize the answer for an RTL terminal: keep Hebrew paragraphs mostly Hebrew, and put English/code identifiers/paths/commands in separate code spans or separate lines when practical. Avoid wrapping English terms in Hebrew parentheses; prefer dashes or a separate sentence. Avoid numbered-list items that start with English/code; start each item with Hebrew text. When mixing Hebrew with English words, code identifiers, paths, or numbers, always insert a space between the Hebrew text and the foreign token (write 'הקובץ src/index.ts' not 'הקובץsrc/index.ts'). Place demonstratives like הזה / הזאת / האלה AFTER the foreign noun with a space (write 'ה-endpoint הזה' not 'הזה-endpoint'). The 'ה-' prefix attaches to a single foreign noun directly via hyphen with no space (e.g. 'ה-API', 'ה-backend'); other Hebrew words must be space-separated from foreign tokens." ;;
