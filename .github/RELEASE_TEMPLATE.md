...
Run **Claude Code** in a terminal that shows **Hebrew, Arabic, Persian, Urdu** and other right‑to‑left languages correctly — on **Windows** (via WSL) and **Linux**.
...
## 🐧 Linux
Download the `kivun-terminal-linux-*.tar.gz` below, extract it, and run `./install.sh`.
