<p align="center">
  <img src="https://github.com/noambrand/kivun-terminal-wsl/releases/download/v1.4.17/Kivun_Terminal_v1.4.13.gif" width="700" alt="Kivun Terminal demo — Hebrew RTL Claude Code session">
</p>
...
> 📹 Prefer to watch first? **[Play the demo video (MP4)](https://github.com/noambrand/kivun-terminal-wsl/releases/download/v1.4.17/Kivun_Terminal_v1.4.13.mp4)**
...
## 🐧 Linux
Download `kivun-terminal-linux-1.4.17.tar.gz` below, extract, and run `./install.sh`.
...
Smoother first‑time setup on PCs that don’t have WSL yet:
- **One‑click WSL install now actually starts.** On a PC without Windows Subsystem for Linux, the installer offered to set it up but the step could silently fail to start — even when you ran it as administrator. That’s fixed: it now launches Microsoft’s `wsl --install` correctly, so you get WSL set up, reboot, and run the installer once more to finish.
